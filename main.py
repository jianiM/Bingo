...
from models.sageconv import SAGENet
from models.fgm import FGM_GCN, FGM_GAT,FGM_SAGE
from utils import * 
import os 
...
def adversarial_training(gnn_model,train_loader,device,optimizer,loss_fn,fgm_model):            
    epoch_loss = 0.0 
    epoch_loss_adv = 0.0 
    train_num = 0.0 
    gnn_model.train()
    for train_idx, batch_train_data in enumerate(train_loader):
        ...
        loss.backward()
        fgm_model.attack() 
        out_attack = gnn_model(batch_train_data).view(-1) 
    ...
    for i in range(n_splits):
        ...
        if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
            ...
            if modelling == "gcn":                
                ...
                fgm_model = FGM_GCN(gnn_model,bias_one="gcnconv1.bias",weight_one="gcnconv1.lin.weight",bias_two="gcnconv2.bias",weight_two="gcnconv2.lin.weight")
            
            elif modelling == "gat":
                gnn_model = GATNet(esm_embeds=1280,n_heads=2,drop_prob=drop_prob,n_output=n_output).to(device)
                fgm_model = FGM_GAT(gnn_model,bias_one="gcn1.bias",weight_one="gcn1.lin_src.weight",bias_two="gcn2.bias",weight_two="gcn2.lin_src.weight")
            
            else:
                gnn_model = SAGENet(esm_embeds=1280,drop_prob=drop_prob,n_output=n_output).to(device)
                fgm_model = FGM_SAGE(gnn_model,bias_one="gcnconv1.lin_l.bias",weight_one="gcnconv1.lin_l.weight",bias_two="gcnconv2.lin_l.bias",weight_two="gcnconv2.lin_r.weight")
            ...
            for epoch in range(num_epoches):
                ...
                epoch_loss,epoch_loss_adv = adversarial_training(gnn_model,train_loader,device,optimizer,loss_fn,fgm_model)
