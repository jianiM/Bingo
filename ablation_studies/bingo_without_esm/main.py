...
if __name__ == "__main__":
    ...
    for i in range(n_splits):
        ...
        if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
            ...
            # adversarial attack            
            if modelling == "gcn":                
                gnn_model = GCNNet(esm_embeds=22,drop_prob=drop_prob,n_output=n_output).to(device)
                ...
                gnn_model = GATNet(esm_embeds=22,n_heads=2,drop_prob=drop_prob,n_output=n_output).to(device)
