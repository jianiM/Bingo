...
def training(model,train_loader,device,optimizer,loss_fn,src_pad_idx):            
    epoch_loss = 0.0  
    train_num = 0.0 
    model.train()
    for idx, (b_x,b_y) in enumerate(train_loader):
        ...
        src_mask = make_pad_mask(b_x,b_x,src_pad_idx=src_pad_idx).to(device)
        optimizer.zero_grad()
        out = model(b_x,src_mask).view(-1)
...
def predicting(test_loader,model,device,src_pad_idx):
    model.eval()
    ...
    with torch.no_grad():
        for idx,(batch_test_data,batch_test_label) in enumerate(test_loader):
            ...
            src_mask = make_pad_mask(batch_test_data,batch_test_data,src_pad_idx=src_pad_idx).to(device)
            output = model(batch_test_data,src_mask)
...
if __name__ == "__main__": 
    ...
    d_model = config.d_model_topofallfeature
    ffn_hidden = config.ffn_hidden_topofallfeature 
    n_head = config.n_head_topofallfeature 
    n_layers = config.n_layers_topofallfeature   
    ...
    src_pad_idx = config.src_pad_idx_topofallfeature 
    ...
    # build dictionary 
#    gene_list_path = "/home/amber/Bingo_balanced/data/celegans/orig_sample_list/gene_list.txt"    
...
#    torch.save(ws,"/home/amber/Bingo_balanced/data/celegans/orig_sample_list/amino_acid_dic.pkl")
    
    #pickle.dump(ws,open("/home/amber/Bingo_balanced/data/celegans/orig_sample_list/amino_acid_dic.pkl","wb")) 
    ...
    for i in range(n_splits):    
        ...
        transformer = Encoder(enc_voc_size=dict_len, max_len=max_len, d_model = d_model, ffn_hidden=ffn_hidden, n_head=n_head, n_layers=n_layers, drop_prob=drop_prob, device=device).to(device) 
        ...
        for epoch in range(num_epoches):
            ...
            epoch_loss = training(model=transformer,train_loader=train_loader,device=device,optimizer=optimizer,loss_fn=loss_fn,src_pad_idx=src_pad_idx) 
            print('epoch_loss:',epoch_loss)
            val_TP,val_FP,val_FN,val_TN,val_fpr,val_tpr,val_auc, val_aupr,val_f1_score, val_accuracy, val_recall, val_specificity, val_precision = predicting(test_loader=valid_loader,model=transformer,device=device,src_pad_idx=src_pad_idx) 
            if val_aupr > best_val_aupr: 
                print("val_auc:",val_auc)
                print('val_aupr:',val_aupr)
                torch.save(transformer, model_file_path)
                best_val_aupr = val_aupr 
    
        checkpoint = torch.load(model_file_path)
        TP,FP,FN,TN,fpr,tpr,auc,aupr,f1_score, accuracy, recall, specificity, precision = predicting(test_loader=test_loader,model=checkpoint,device=device,src_pad_idx=src_pad_idx)       
        test_auc[i] = auc
        test_aupr[i] = aupr
