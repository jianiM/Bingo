...
if __name__ == "__main__": 
    ...
    channels = config.channels_topofallfeature  
    kernel = config.kernel_topofallfeature   
    fc_dims = [16*12,32]
    ...
    for i in range(n_splits):    
        ...
        cnn_model = CNNBlock(dict_len=dict_len,embed_dim=embed_dim,pad_idx=pad_idx,n_output=n_output,channels=channels,kernel=kernel,drop_prob=drop_prob,fc_dims=fc_dims).to(device)
        ...
        for epoch in range(num_epoches):
           ...
           # adversarial training function  
            epoch_loss = training(model=cnn_model,train_loader=train_loader,device=device,optimizer=optimizer,loss_fn=loss_fn)
            print('epoch_loss:',epoch_loss)
            val_TP,val_FP,val_FN,val_TN,val_fpr,val_tpr,val_auc, val_aupr,val_f1_score, val_accuracy, val_recall, val_specificity, val_precision = predicting(test_loader=valid_loader,model=cnn_model,device=device) 
            if val_aupr > best_val_aupr: 
                print("val_auc:",val_auc)
                print('val_aupr:',val_aupr)
                torch.save(cnn_model, model_file_path)
