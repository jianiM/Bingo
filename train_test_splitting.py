...
def generate_fold_dataset(n_splits,pos_samples_path,neg_samples_path,kfold_root_path):
    ...
    '''
    pos_samples = pd.read_excel(pos_samples_path)['GeneSymbol'].values
    neg_samples = pd.read_excel(neg_samples_path)['GeneSymbol'].values
    ...
    for i in range(n_splits):
        ...
        fold_train_data_df = pd.DataFrame(batch_train_samples,columns=['GeneSymbol'])
        fold_test_data_df = pd.DataFrame(batch_test_samples,columns=['GeneSymbol']) 
